...
export default function InputText(props: {label: string; value: Dispatch<SetStateAction<string>>}) {
    ...
    return <div className="relative my-4">
        ...
        <input type="text" id={props.label} className="w-full bg-white border-[1.5px] border-black outline-none p-3 rounded-lg" onFocus={() => {setFocus(true)}} onBlur={() => {setFocus(false); if(value != '') setFocus(true)}} value={value} onChange={(event) => {setValue(event.currentTarget.value); props.value(event.currentTarget.value)}}/>
    </div>
}