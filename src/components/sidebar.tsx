...
export default function SideBar(props: {index: Dispatch<SetStateAction<number>>; expand: Dispatch<SetStateAction<boolean>>; expanded: boolean; loading: Dispatch<SetStateAction<boolean>>;}) {
    ...
    return <div className={`transition-all ease-in-out duration-200 bg-white fixed top-6 bottom-6 left-6 rounded-xl shadow-md py-4 px-4 flex flex-col justify-between z-40 ${props.expanded ? 'w-60 items-start' : 'w-fit items-center'}`}>
        <div className="absolute top-1/2 -translate-y-1/2 -right-10 px-2 py-6 bg-white rounded-lg shadow-md" onClick={() => {props.expanded ? props.expand(false) : props.expand(true)}}>{props.expanded ? <CaretLeft weight={'bold'}/> : <CaretRight weight={'bold'}/>}</div>
